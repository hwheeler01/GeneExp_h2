...
for(h in 1:length(tislist)){
  ...
  for(i in 1:length(localensid)){
    ...
    ## Y ~ localGRM
    runLOC <- "gcta64 --grm " %&% grm.dir %&% ensid %&% " --reml --pheno tmp.TS.pheno." %&% thresh %&% gencodeset %&% " --out tmp.TS." %&% thresh %&% gencodeset
    system(runLOC)
    ...
    runGLO <- "gcta64 --grm " %&% grm.trans.dir %&% "global-" %&% ensid %&% "-otherChr --reml --pheno tmp.TS.pheno." %&% thresh %&% gencodeset %&% " --out tmp.TS." %&% thresh %&% gencodeset
    system(runGLO)
    hsq <- scan("tmp.TS." %&% thresh %&% gencodeset %&% ".hsq","character")
    ...
    system(runMULT)
    system(runMULT2)
    runLOCGLO <- "gcta64 --mgrm-bin tmp.TS.multiGRM." %&% thresh %&% gencodeset %&% " --reml --pheno tmp.TS.pheno." %&% thresh %&% gencodeset %&% " --out tmp.TS." %&% thresh %&% gencodeset
    system(runLOCGLO)
    hsq <- scan("tmp.TS." %&% thresh %&% gencodeset %&% ".hsq","character")
    if(hsq[18]=="logL0"){
  ...
  write.table(full.mat,file="GTEx.TS." %&% tis %&% ".h2.all.models_FHS" %&% thresh %&% ".Chr" %&% gencodeset %&% "_globalOtherChr." %&% date %&% ".txt",quote=F,row.names=F,sep="\t")
