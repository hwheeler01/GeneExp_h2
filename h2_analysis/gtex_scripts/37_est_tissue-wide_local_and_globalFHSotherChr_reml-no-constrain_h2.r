...
for(h in 1:length(tislist)){
  ...
  for(i in 1:length(localensid)){
    ...
    ## Y ~ localGRM
    runLOC <- "gcta64 --grm " %&% grm.dir %&% ensid %&% " --reml-no-constrain --pheno tmp.TW.pheno." %&% thresh %&% gencodeset %&% " --out tmp.TW." %&% thresh %&% gencodeset
    system(runLOC)
    if(file.exists("tmp.TW." %&% thresh %&% gencodeset %&% ".hsq")==TRUE){
    ...
    loc.mat[i,] <- res
    system("rm tmp.TW." %&% thresh %&% gencodeset %&% ".hsq")    
    ...
    runGLO <- "gcta64 --grm " %&% grm.trans.dir %&% "global-" %&% ensid %&% "-otherChr --reml-no-constrain --pheno tmp.TW.pheno." %&% thresh %&% gencodeset %&% " --out tmp.TW." %&% thresh %&% gencodeset
    system(runGLO)
    if(file.exists("tmp.TW." %&% thresh %&% gencodeset %&% ".hsq")==TRUE){
      hsq <- scan("tmp.TW." %&% thresh %&% gencodeset %&% ".hsq","character")
    ...
    system(runMULT)
    system(runMULT2)
    runLOCGLO <- "gcta64 --mgrm-bin tmp.TW.multiGRM." %&% thresh %&% gencodeset %&% " --reml-no-constrain --pheno tmp.TW.pheno." %&% thresh %&% gencodeset %&% " --out tmp.TW." %&% thresh %&% gencodeset
    system(runLOCGLO)
    if(file.exists("tmp.TW." %&% thresh %&% gencodeset %&% ".hsq")==TRUE){
      hsq <- scan("tmp.TW." %&% thresh %&% gencodeset %&% ".hsq","character")
  ...
  write.table(full.mat,file="GTEx.TW." %&% tis %&% ".h2.all.models_FHS" %&% thresh %&% ".Chr" %&% gencodeset %&% "_globaleQTLOtherChr_reml-no-constrain." %&% date %&% ".txt",quote=F,row.names=F,sep="\t")
