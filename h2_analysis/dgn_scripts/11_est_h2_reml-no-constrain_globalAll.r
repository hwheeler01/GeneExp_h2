...
for(i in 1:length(localensid)){
	...
	## Y ~ globalGRM (all SNPs)
	runGLO <- "gcta64 --grm " %&% my.dir %&% "dgn-grms/DGN.global_Chr1-22 --reml-no-constrain --pheno tmp.pheno." %&% thresh %&% gencodeset %&% " --out tmp." %&% thresh %&% gencodeset
	system(runGLO)
	if(file.exists("tmp." %&% thresh %&% gencodeset %&% ".hsq")==TRUE){
		hsq <- scan("tmp." %&% thresh %&% gencodeset %&% ".hsq","character")
	...
	runMULT2 <- "echo " %&% my.dir %&% "dgn-grms/DGN.global_Chr1-22 >> tmp.multiGRM." %&% thresh %&% gencodeset
	system(runMULT)
        system(runMULT2)
...
write.table(full.mat,file=tis %&% ".h2.all.models_FHS" %&% thresh %&% ".Chr" %&% gencodeset %&% "_globalAll_reml-no-constrain." %&% date %&% ".txt",quote=F,row.names=F,sep="\t")
