...
for(tis in tislist){
  ...
  glo.jt <- select(data,tissue,glo.jt.h2,glo.jt.se)  %>% mutate(ymin = pmax(0, glo.jt.h2 - 2 * glo.jt.se), ymax = pmin(1, glo.jt.h2 + 2 * glo.jt.se)) %>% mutate(glo.jt.P = 2*pnorm(-abs(glo.jt.h2/glo.jt.se))) %>%  mutate(glo.jt.Q = p.adjust(glo.jt.P,method="BH")) %>% mutate(gloQlt05=glo.jt.Q < 0.1) %>% arrange(glo.jt.h2) %>% mutate(place=1:length(data$tissue)) 
  ts <- rbind(ts,glo.jt)
}

ts <- mutate(ts, `q-value`=factor(gloQlt05,labels=c('\u2265 0.1')))

p<-ggplot(ts[complete.cases(ts),],aes(x=place,y=glo.jt.h2,ymin=ymin,ymax=ymax,color=`q-value`) ) + facet_wrap(~tissue,ncol=3) + geom_pointrange(col='gray')+geom_point()+ylab(expression("global h"^2)) + xlab(expression("genes ordered by joint global h"^2))+theme_bw() + coord_cartesian(ylim=c(0,1))

png(filename=fig.dir %&% "Fig-GTEx_TW_glo_jt_h2.png",width=720,height=480)
p
dev.off()
tiff(filename=fig.dir %&% "Fig-GTEx_TW_glo_jt_h2.tiff",width=720,height=480)
p
dev.off()
```
