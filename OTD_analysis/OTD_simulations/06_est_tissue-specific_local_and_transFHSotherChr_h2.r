...
idfile <- lmer.dir %&% "resid_" %&% sim %&% "_SAMPID"
genefile <- lmer.dir %&% "resid_" %&% sim %&% "_GENE"
expfile <- lmer.dir %&% "resid_" %&% sim %&% "_SAMPIDxGENE"
...
for(h in 1:length(tislist)){
  ...
  for(i in 1:length(localensid)){
    ...
    write.table(geneexp, file="tmp.TS.pheno." %&% thresh %&% gencodeset %&% errvar %&% mult, col.names=F, quote=F) #output pheno for gcta
    
    ## Y ~ localGRM
    runLOC <- "gcta64 --grm " %&% grm.dir %&% ensid %&% " --reml --pheno tmp.TS.pheno." %&% thresh %&% gencodeset %&% errvar %&% mult %&% " --out tmp.TS." %&% thresh %&% gencodeset %&% errvar %&% mult
    system(runLOC)
    if(file.exists("tmp.TS." %&% thresh %&% gencodeset %&% errvar %&% mult %&% ".hsq")==TRUE){
        hsq <- scan("tmp.TS." %&% thresh %&% gencodeset %&% errvar %&% mult %&% ".hsq","character")
    ...
    loc.mat[i,] <- res
    system("rm tmp.TS." %&% thresh %&% gencodeset %&% errvar %&% mult %&% ".hsq")
    ...
    write.table(grmmat,file="tmp.TS.grm.list" %&% thresh %&% gencodeset %&% errvar %&% mult,quote=F,col.names=F,row.names=F)
    runGRM <- "gcta64 --mgrm-bin tmp.TS.grm.list" %&% thresh %&% gencodeset %&% errvar %&% mult %&% " --make-grm --out " %&% grm.trans.dir %&% "global-" %&% ensid %&% "-otherChr"
    system(runGRM)
    
    runGLO <- "gcta64 --grm " %&% grm.trans.dir %&% "global-" %&% ensid %&% "-otherChr --reml --pheno tmp.TS.pheno." %&% thresh %&% gencodeset %&% errvar %&% mult %&% " --out tmp.TS." %&% thresh %&% gencodeset %&% errvar %&% mult
    system(runGLO)
    if(file.exists("tmp.TS." %&% thresh %&% gencodeset %&% errvar %&% mult %&% ".hsq")==TRUE){
	hsq <- scan("tmp.TS." %&% thresh %&% gencodeset %&% errvar %&% mult %&% ".hsq","character")
    ...
    runMULT <- "echo " %&% grm.dir %&% ensid %&% " > tmp.TS.multiGRM." %&% thresh %&% gencodeset %&% errvar %&% mult
    runMULT2 <- "echo " %&% grm.trans.dir %&% "global-" %&% ensid %&% "-otherChr >> tmp.TS.multiGRM." %&% thresh %&% gencodeset %&% errvar %&% mult
    system(runMULT)
    system(runMULT2)
    runLOCGLO <- "gcta64 --mgrm-bin tmp.TS.multiGRM." %&% thresh %&% gencodeset %&% errvar %&% mult %&% " --reml --pheno tmp.TS.pheno." %&% thresh %&% gencodeset %&% errvar %&% mult %&% " --out tmp.TS." %&% thresh %&% gencodeset %&% errvar %&% mult
    system(runLOCGLO)
    if(file.exists("tmp.TS." %&% thresh %&% gencodeset %&% errvar %&% mult %&% ".hsq")==TRUE){
        hsq <- scan("tmp.TS." %&% thresh %&% gencodeset %&% errvar %&% mult %&% ".hsq","character")
  ...
  write.table(full.mat,file="GTEx.TS." %&% tis %&% "_" %&% sim %&% ".h2.all.models_FHS" %&% thresh %&% ".Chr" %&% gencodeset %&% "_globaleQTLOtherChr." %&% date %&% ".txt",quote=F,row.names=F,sep="\t")
